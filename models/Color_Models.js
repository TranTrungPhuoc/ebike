...
const Schema = require('../schemas/Color_Schema')
const Category_Model = require('../schemas/Category_Schema')
const mongoose = require('mongoose');
class Color_Models extends Models{
    constructor(table){
        super(table)
        this.table = Schema
    }
    ...
    async getRelative(slug){
        const Color = await this.table.find({slug}).exec();
        if(Color.length > 0){
            return await Category_Model.aggregate([
                {$match: {_id: Color[0].parentID} },
                {$sort: {created: -1}},
                {
                    $lookup: {
                        from: 'Colors',
...
module.exports = new Color_Models