...
const Schema = require('../schemas/Delivery_Schema')
const Category_Model = require('../schemas/Category_Schema')
const mongoose = require('mongoose');
class Delivery_Models extends Models{
    constructor(table){
        super(table)
        this.table = Schema
    }
    ...
    async getRelative(slug){
        const Delivery = await this.table.find({slug}).exec();
        if(Delivery.length > 0){
            return await Category_Model.aggregate([
                {$match: {_id: Delivery[0].parentID} },
                {$sort: {created: -1}},
                {
                    $lookup: {
                        from: 'Deliverys',
...
module.exports = new Delivery_Models