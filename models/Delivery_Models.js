...
const Schema = require('../schemas/Payment_Schema')
const Category_Model = require('../schemas/Category_Schema')
const mongoose = require('mongoose');
class Payment_Models extends Models{
    constructor(table){
        super(table)
        this.table = Schema
    }
    ...
    async getRelative(slug){
        const Payment = await this.table.find({slug}).exec();
        if(Payment.length > 0){
            return await Category_Model.aggregate([
                {$match: {_id: Payment[0].parentID} },
                {$sort: {created: -1}},
                {
                    $lookup: {
                        from: 'Payments',
...
module.exports = new Payment_Models