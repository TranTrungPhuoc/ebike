...
class Order_Api extends Api {
    ...
    async getList(){
        const data = await Order_Models.m_getList()
        this.res.send({
            code: 200,
            message: "Success",
            response: data
    ...
    async new(){
        ...
        const data = await Order_Models.m_new(type.trim())
    ...
    async getRelative() {
        ...
        const data = await Order_Models.getRelative(slug.trim())

        for (let index = 0; index < data.length; index++) {
            const element = data[index];
            for (let j = 0; j < element['Orders'].length; j++) {
                const element2 = element['Orders'][j];
                element2['avatar'] = element2['avatar'] != '' ? this.req.protocol + '://' + this.req.headers.host + '/uploads/Order/' + element2['avatar'] : '';
            }
        }
    ...
    async viewMore() {
        ...
        const data = await Order_Models.viewMore(limit != undefined ? parseInt(limit) : 10)

        for (let index = 0; index < data.length; index++) {
            const element = data[index];
            element['avatar'] = element['avatar'] != '' ? this.req.protocol + '://' + this.req.headers.host + '/uploads/Order/' + element['avatar'] : '';
        }
    ...
    async feature() {
        const data = await Order_Models.feature()
    ...
    async getDetailSlug() {
        ...
        const data = await Order_Models.getDetailSlug(slug.trim())

        this.res.send({data})
        return

        data[0]['avatar'] = data[0]['avatar'] != '' ? this.req.protocol + '://' + this.req.headers.host + '/uploads/Order/' + data[0]['avatar'] : '';
    ...
    async view() {
        ...
        const check_exist_db = await Order_Models.check(this.req.params.id);
        ...
        const data = await Order_Models.view(this.req.params.id, check_exist_db[0].view + 1)
    ...
    async search() {
        ...
        const data = await Order_Models.search(key, parseInt(page ? (page == 1 ? 0 : page) : 0), parseInt(limit ?? 10))
        for (let index = 0; index < data.length; index++) {
            const element = data[index];
            element['avatar'] = element['avatar'] != '' ? this.req.protocol + '://' + this.req.headers.host + '/uploads/Order/' + element['avatar'] : '';
        }
    ...
    async insert(){
        ...
        const response = await Order_Models.m_insert(this.req.body);
        this.res.send({
            code: 200,
            message: "Success",
            response
