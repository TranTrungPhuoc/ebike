...
const Schema = require('../schemas/Pin_Schema')
const Category_Model = require('../schemas/Category_Schema')
const mongoose = require('mongoose');
class Pin_Models extends Models{
    constructor(table){
        super(table)
        this.table = Schema
    }
    ...
    async getRelative(slug){
        const Pin = await this.table.find({slug}).exec();
        if(Pin.length > 0){
            return await Category_Model.aggregate([
                {$match: {_id: Pin[0].parentID} },
                {$sort: {created: -1}},
                {
                    $lookup: {
                        from: 'Pins',
